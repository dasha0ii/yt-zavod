...
Проект настроен для легкого деплоя:
...
2. **Загрузите на GitHub/GitLab:**
   - Создайте репозиторий на платформе
   - Добавьте remote и push

3. **На сервере:**
   - Склонируйте репозиторий
   - Создайте `.env` файл с токенами
   - Добавьте `client_secrets.json`
   - Запустите `docker-compose up --build`

Файлы `.env`, `client_secrets.json`, `output/`, `sources/` и базы данных исключены из Git через `.gitignore`.
